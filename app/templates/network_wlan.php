        ...
        <fieldset>
            <div class="form-group form-actions">
                <div class="col-sm-12">
                    ...
                    <input type="hidden" name="wifiprofile[ssid]" value="<?=$this->uri(4) ?>">
        ...
        <fieldset>
            <?php if($this->stored !== 1): ?>
                ...
                <div class="form-group <?php if($this->uri(4) !== 'add'): ?>hide<?php endif; ?>">
                    ...
                    <div class="col-sm-10">
                        <input class="form-control input-lg" type="text" id="wifi-ssid" name="nic[ssid]" value="<?php if($this->uri(4) !== 'add'): ?><?=$this->uri(4) ?><?php endif; ?>" data-trigger="change" autocomplete="off">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                        <select id="wifi-security" name="nic[encryption]" class="selectpicker" data-style="btn-default btn-lg">
                            <option value="none" <?php if($this->uri(4) !== null && strpos($this->{$this->uri(4)}->{'Encryption key'}, 'off') OR $this->profile_{$this->uri(4)}->encryption === 'open'): ?>selected<?php endif; ?>>none (Open network)</option>
                ...
                <div id="wifi-security-key" class="form-group hide">
                    ...
                    <div class="col-sm-10">
                        ...
                        <input type="hidden" name="nic[id]" value="<?=$this->profile_{$this->uri(4)}->id ?>">
                        <?php endif; ?>
                        <input class="form-control input-lg" type="password" id="wifi-password" name="nic[key]" value="<?=$this->profile_{$this->uri(4)}->key ?>" data-trigger="change" autocomplete="off">
            ...
            <?php if($this->stored !== 1): ?>
                ...
                <div id="network-manual-config" class="hide">        
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input class="form-control input-lg" type="text" pattern="((^|\.)((25[0-5])|(2[0-4]\d)|(1\d\d)|([1-9]?\d))){4}$" id="address" name="nic[ip]" <?php if (isset($this->nic_stored)): ?>value="<?=$this->nic_stored->ip ?>" placeholder="<?=$this->nic_stored->ip ?><?php else: ?>value="0.0.0.0" placeholder="<?=$this->nic->ip ?>"<?php endif; ?> data-parsley-trigger="change" required />
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input class="form-control input-lg" type="text" pattern="((^|\.)((25[0-5])|(2[0-4]\d)|(1\d\d)|([1-9]?\d))){4}$" id="netmask" name="nic[netmask]" <?php if (isset($this->nic_stored)): ?>value="<?=$this->nic_stored->netmask ?>" data-parsley-trigger="change" placeholder="<?=$this->nic_stored->netmask ?>"<?php else: ?>value="0.0.0.0" data-parsley-trigger="change" placeholder="<?=$this->nic->netmask ?>"<?php endif; ?> required />
        ...
        <div class="form-group form-actions">
            <div class="col-sm-offset-2 col-sm-10">
                ...
                <?php elseif($this->stored === 1): ?>
                    ...
                    <input type="hidden" name="nic[newssid]" value="<?=$this->uri(4) ?>">
                <?php else: ?>
                    <!-- Save profile -->
                    <button type="submit" class="btn btn-primary btn-lg" name="nic[nic]" value="<?=$this->uri(3) ?>">Save profile</button>
                    <input type="hidden" name="nic[currentssid]"           value="<?=$this->nic->currentssid ?>">
                    <input type="hidden" name="nic[newssid]"               value="<?=$this->uri(4) ?>">
                    <input type="hidden" name="nic[GroupCipher]"           value="<?=$this->{$this->uri(4)}->{'Group Cipher'} ?>">
                    <input type="hidden" name="nic[PairwiseCiphers1]"      value="<?=$this->{$this->uri(4)}->{'Pairwise Ciphers (1)'} ?>">
                    <input type="hidden" name="nic[PairwiseCiphers2]"      value="<?=$this->{$this->uri(4)}->{'Pairwise Ciphers (2)'} ?>">
                    <input type="hidden" name="nic[EncryptionKey]"         value="<?=$this->{$this->uri(4)}->{'Encryption key'} ?>">
                    <input type="hidden" name="nic[AuthenticationSuites1]" value="<?=$this->{$this->uri(4)}->{'Authentication Suites (1)'} ?>">
                    <input type="hidden" name="nic[AuthenticationSuites2]" value="<?=$this->{$this->uri(4)}->{'Authentication Suites (2)'} ?>">
                    <input type="hidden" name="nic[ie]"                    value="<?=$this->{$this->uri(4)}->{'IE'} ?>">
                <?php endif; ?>
            </div>
        </div>
        <fieldset>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <input type="hidden" name="wifiprofile[ssid]" value="<?=$this->uri(4) ?>">
                    <input type="hidden" name="wifiprofile[id]" value="<?=$this->uri(4) ?>">
                    <input type="hidden" name="wifiprofile[nic]" value="<?=$this->uri(3) ?>">
                </div>
            </div>
        </div>
