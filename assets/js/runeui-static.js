...
var settings = new RuneModule('mock-response-settings.json');


var credits = new RuneModule('mock-response-settings.json');
var debug = new RuneModule('mock-response-settings.json');
var dev = new RuneModule('mock-response-settings.json');
var error = new RuneModule('mock-response-settings.json');

var network = new RuneModule('mock-response-settings.json');
var sources = new RuneModule('mock-response-settings.json');
...
////////////mpd.vm = {}
////////////mpd.vm.init = function (url) {
////////////    this.url = url;
////////////    mpd.getData(url)
////////////}
////////////mpd.vm.save = function () {
////////////    console.log(mpd.vm.url);
////////////    console.log(mpd.data);
////////////}

////////////mpd.getData = function (url) {
////////////    // 'mock-response-mpd.json'
////////////    return m.request({ method: 'GET', url: url }).then(function (response) {
////////////        mpd.data = response;
////////////    });
////////////};

////////////mpd.controller = function () {
////////////    mpd.vm.init('mock-response-mpd.json');
    ...
    return [m('h1', 'MPD Configuration'), '\n', m('p', ['\n    If you mess up with this configuration you can ', m('a[data-toggle="modal"][href="#mpd-config-defaults"]', 'reset to default'), '.\n']), '\n', m('form.form-horizontal[action=""][method="post"]', [
		...
		m('fieldset', [
			...
			m('.form-group', [
				...
				m('.col-sm-10', [
					m('select[data-style="btn-default btn-lg"][id="log-level"][name="conf[user]"]', { config: selectpicker }, [
			...
			m('.form-group', [
				...
				m('.col-sm-10', [
					m('select[data-style="btn-default btn-lg"][id="log-level"][name="conf[log_level]"]', { config: selectpicker }, [
			...
			m('.form-group', [
				...
				m('.col-sm-10', [
                    ...
                    createSelectYesNo('log-level', mpd.vm.data.conf, 'log-level', selectpicker),
			...
			m('.form-group', [
				...
				m('.col-sm-10', [
					createSelectYesNo('ffmpeg', mpd.vm.data.conf, 'ffmpeg', selectpicker),
			...
			m('.form-group', [
				...
				m('.col-sm-10', [
					createSelectYesNo('gapless-mp3-playback', mpd.vm.data.conf, 'gapless-mp3-playback', selectpicker),
			...
			m('.form-group', [
				...
				m('.col-sm-10', [
					createSelectYesNo('dsd-usb', mpd.vm.data.conf, 'dsd-usb', selectpicker),
			...
			m('.form-group', [
				...
				m('.col-sm-10', [
					createSelectYesNo('volume-normalization', mpd.vm.data.conf, 'volume-normalization', selectpicker),
			...
			m('.form-group', [
				...
				m('.col-sm-10', [
					m('input.form-control.input-lg[data-trigger="change"][id="audio-buffer-size"][min="512"][name="conf[audio_buffer_size]"][type="number"]', bind2(mpd.vm.data, 'audio_buffer_size')),
			...
			m('.form-group', [
				...
				m('.col-sm-10', [
					m('select[data-style="btn-default btn-lg"][id="buffer-before-play"][name="conf[buffer_before_play]"]', { config: selectpicker }, [
			...
			m('.form-group', [
				...
				m('.col-sm-10', [
					createSelectYesNo('auto-update', mpd.vm.data.conf, 'auto-update', selectpicker),
